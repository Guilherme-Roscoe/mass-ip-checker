...
async def scan_ip(ip):
    ...
    # Filtra o erro para fazer um novo teste
    errored_blacklists = [r for r in results if r["status"] == "ERRO"]
    if errored_blacklists:
        #print(f"🔄 Retentando {len(errored_blacklists)} blacklists que deram erro...")
        # Tenta novamente as blacklists que deram erro
        retry_tasks = [check_dnsbl(ip, bl["blacklist"]) for bl in errored_blacklists]
        retry_results = await asyncio.gather(*retry_tasks)
        for retry in retry_results:
            results = [result if result["blacklist"] != retry["blacklist"] else retry for result in results]
